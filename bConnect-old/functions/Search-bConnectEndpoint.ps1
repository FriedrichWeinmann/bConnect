...
function Search-bConnectEndpoint {
	...
	(
		...
		[bConnectEndpointType]$Type,
		[Parameter(ParameterSetName = 'Search',
				   Position = 3)]
		[switch]$OnlyActiveClients,
	...
	process {
		...
		switch ($PSCmdlet.ParameterSetName) {
			"Search" {
				...
				$Result = Invoke-bConnectGet -Controller "Search" -Data $_body | Where-Object -FilterScript $WhereBlock | Select-Object  @{ Name = "EndpointGuid"; Expression = { $_.ID } }, ID,Name, AdditionalInfo, @{ Name = "Type"; Expression = { [bConnectSearchResultType]$_.Type } }, Deactivated
			}
			"Filter" {
				try {
					$Result = Get-bConnectEndpoint | Where-Object -FilterScript $WhereBlock -ErrorAction Stop | Select-Object  EndpointGuid, ID, @{ Name = "Name"; Expression = { $_.DisplayName } }, @{ Name = "AdditionalInfo"; Expression = { $_.Comments } }, @{ Name = "Type"; Expression = { [bConnectEndpointType]$_.Type } }, Deactivated
				}
				catch {
					$ErrorObject = New-Object System.Exception "Error with the Filter $($Filter.ToString()) . Please Check Syntax. $_"
					throw $ErrorObject
