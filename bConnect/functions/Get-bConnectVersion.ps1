...
#>
    ...
    if ($info)
    {
        ...
        $bmsVersionItem = [System.Version]::Parse($info.bMSVersion)
        Write-PSFMessage -Level Verbose -Message "Versions detected: bConnect: $($bcVersion) | BMS: $($bmsVersionItem)"
        switch -Wildcard ($bmsVersionItem)
        {
            "14.0.*"
            {
