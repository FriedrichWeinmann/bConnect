...
#>
    ...
    process
    {
        If (Test-Guid $StaticGroup.Id)
        {
            ...
            if (Test-PSFShouldProcess -PSCmdlet $PSCmdlet -Target $newGroupGet.Id -Action 'Edit Static Group')
            {
                Invoke-bConnectPatch -Controller "StaticGroups" -objectGuid $newGroup.Id -Data $newGroup | Add-ObjectDetail -TypeName 'bConnect.StaticGroup' -WithID
            }
            else
            {
